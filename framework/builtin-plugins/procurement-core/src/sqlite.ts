export type ProcurementCoreSqliteOptions = {
  tablePrefix?: string;
};

export function buildProcurementCoreSqliteMigrationSql(options: ProcurementCoreSqliteOptions = {}): string[] {
  const tablePrefix = normalizePrefix(options.tablePrefix ?? "procurement_core_");
  return [
    ...
    `CREATE UNIQUE INDEX IF NOT EXISTS ${getProcurementCoreSqliteLookupIndexName(tablePrefix)} ON ${tablePrefix}primary_records (tenant_id, title, correlation_id);`,
    `CREATE INDEX IF NOT EXISTS ${getProcurementCoreSqliteStatusIndexName(tablePrefix)} ON ${tablePrefix}exception_records (tenant_id, status, severity);`
  ];
}

export function buildProcurementCoreSqliteRollbackSql(options: ProcurementCoreSqliteOptions = {}): string[] {
  const tablePrefix = normalizePrefix(options.tablePrefix ?? "procurement_core_");
  return [
...
export function getProcurementCoreSqliteLookupIndexName(tablePrefix = "procurement_core_"): string {
  return `${normalizePrefix(tablePrefix)}primary_lookup_idx`;
}

export function getProcurementCoreSqliteStatusIndexName(tablePrefix = "procurement_core_"): string {
