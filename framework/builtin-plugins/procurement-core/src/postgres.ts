export type ProcurementCoreSqlOptions = {
  schemaName?: string;
  dropSchema?: boolean;
};

export function buildProcurementCoreMigrationSql(options: ProcurementCoreSqlOptions = {}): string[] {
  const schemaName = normalizeIdentifier(options.schemaName ?? "procurement_core", "schemaName");
  return [
    ...
    `CREATE UNIQUE INDEX IF NOT EXISTS ${getProcurementCoreLookupIndexName()} ON ${schemaName}.primary_records (tenant_id, title, correlation_id);`,
    `CREATE INDEX IF NOT EXISTS ${getProcurementCoreStatusIndexName()} ON ${schemaName}.exception_records (tenant_id, status, severity);`
  ];
}

export function buildProcurementCoreRollbackSql(options: ProcurementCoreSqlOptions = {}): string[] {
  const schemaName = normalizeIdentifier(options.schemaName ?? "procurement_core", "schemaName");
  const dropSchema = options.dropSchema ?? schemaName !== "procurement_core";
  return [
...
export function getProcurementCoreLookupIndexName(): string {
  return "procurement_core_primary_lookup_idx";
}

export function getProcurementCoreStatusIndexName(): string {
  return "procurement_core_exception_status_idx";
}
