...
import { businessFlowDefinitions, createRequisition, issuePurchaseOrder, requestReceipt, placeRecordOnHold, releaseRecordHold, amendRecord, reverseRecord } from "../../src/flows/catalog";
...
describe("plugin manifest", () => {
  it("keeps a stable package id and business contract surface", () => {
    expect(manifest.id).toBe("procurement-core");
    expect(manifest.kind).toBe("plugin");
    expect(manifest.publicCommands).toContain("procurement.requisitions.create");
    expect(manifest.publicEvents).toContain("procurement.requisition-created.v1");
  });
});
...
describe("domain catalog", () => {
  it("keeps ERPNext parity references and operational surfaces visible", () => {
    ...
    expect(typeof createRequisition).toBe("function");
    expect(typeof issuePurchaseOrder).toBe("function");
    expect(typeof requestReceipt).toBe("function");
